   ...
   /* protected static X509Certificate ObtenerCertificado(String rutaAlmacen, String passwordAlmacen, String nombreCertificado) throws Exception
    ...
    protected static String InternalSign(Document xml, String elementoXmlAFirmar, String elementoXmlPadreDeFirma, Key clavePrivada, X509Certificate certificado) throws Exception
    {
        ...
        {           
            ...
            /****************************************/
            /* Firmamos el xml */
            XMLSignature firma = fac.newXMLSignature(infoFirma, infoClaveCertificado, null, elementoXmlAFirmar, null);
    ...
    private static void makeNodeReferenceID(Node signatureNode,DOMValidateContext context) throws SignatureException
    {
    	Node signedInfo = XMLDOMUtils.getFirstChildNode(signatureNode, "SignedInfo");
		Node nreferido=null;
    	if (signedInfo!=null)
    	{
    		//Buscamos el nodo referencia
    		Node reference = XMLDOMUtils.getFirstChildNode(signedInfo, "Reference");
    		if (reference!=null)
    		{
    			//Sacamos el valor de su URI
    			String valorRef = XMLDOMUtils.getAttributeValue(reference, "URI");
    			if (valorRef!=null)
    			{
    				valorRef = valorRef.substring(1); //Empieza por "#", lo quitamos
    				//Ahora hemos de encontrar un elemento con un atributo cuyo valor sea el 
    				//elemento que hemos encontrado. Sólo tendremos en cuenta los que tengan nombre
    				//"ID", "id" o similar, por lo que no es posible que haya más de uno.
    				NodeList n = XMLDOMUtils.getAllNodesIdAttribute(signatureNode.getOwnerDocument(), valorRef);
    				if (n.getLength()>0)
    				{
    					if (n.getLength()==1)
    					{
    						...
    						NamedNodeMap atributos= nreferido.getAttributes();
    						Node atributo;
    						for (int i=0;i<atributos.getLength();i++)
    						{
    							atributo=atributos.item(i);
    							String nombre=atributo.getLocalName(); 
    							if (nombre.length()>2)
    							{
    								continue;
    							}
    							String valor = atributo.getNodeValue();
    							if ("id".equalsIgnoreCase(nombre) && valorRef.equals(valor))
    							{
    								//Es el que buscamos, lo convertimos en Id
    								context.setIdAttributeNS((Element)nreferido, atributo.getNamespaceURI(), nombre);
    								break; 
    							}
    						}
    					...
    					{
   							throw new SignatureException("Error, se ha encontrado más de un nodo con nombre \"ID\" y valor \""+valorRef+"\"");
    					}
    				}
    			}
    		}
    ...
    protected static boolean InternalValidate(Document xml) throws Exception
    {
        try
        {
            ...
            /***********************************************/
            // Desde Java 1.7 25, no sirve con que la referencia tenga un URI que vaya contra un atributo con nombre "ID" ,
            // sino que el atributo debe estar marcado como tipo "ID" en el documento.
            // Si no, no lo encuentra y no valida la firma.
            // Para esto, debemos recuperar la referencia de firma, y buscar un elemento con un atributo cuyo
            // valor sea el texto de esa referencia, y después registrarlo como ID.
            //Si no podemos, la validación de firma fallará.
            makeNodeReferenceID(nl.item(0), valContext);
            /***********************************************/
            // obtener la firma
